...
var NEVER = INVALID;
// src/schema.ts
var ThemeModeSchema = exports_external.enum(["light", "dark", "system"]);
var StargateConfigSchema = exports_external.object({
  autoConnect: exports_external.boolean().default(true),
  defaultRelay: exports_external.string().optional(),
  trustedNodes: exports_external.array(exports_external.string()).default([])
});
var AeonPreferencesSchema = exports_external.object({
  theme: exports_external.object({
    mode: ThemeModeSchema.default("system"),
    cssOverrides: exports_external.string().optional()
  }).default({}),
  security: exports_external.object({
    disabledApps: exports_external.array(exports_external.string()).default([]),
    disabledMcps: exports_external.array(exports_external.string()).default([]),
    blockedSites: exports_external.array(exports_external.string()).default([])
  }).default({}),
  stargate: StargateConfigSchema.default({}),
  locale: exports_external.object({
    timezone: exports_external.string().optional(),
    language: exports_external.string().optional()
  }).default({}),
  agent: exports_external.object({
    persona: exports_external.string().optional(),
    pronouns: exports_external.string().optional()
  }).default({}),
  flags: exports_external.record(exports_external.boolean()).default({}),
  namespaces: exports_external.record(exports_external.any()).default({}),
...
var PreferencesContext = import_react.createContext(null);
function PreferencesProvider({
  store = new InMemoryPreferencesStore,
  children
}) {
  const [preferences, setPreferences] = import_react.useState(store.getPreferences());
  ...
  return /* @__PURE__ */ jsx_dev_runtime.jsxDEV(PreferencesContext.Provider, {
    value,
    children
  }, undefined, false, undefined, this);
}
function useAeonPreferences() {
  const context = import_react.useContext(PreferencesContext);
  if (!context) {
...
// src/mcp.ts
function createPreferencesMcpTools(store) {
  return [
    {
